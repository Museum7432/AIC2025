...
def search_obj_count_engine_slow(
    ...
    for ins_id, instance in enumerate(db):
        ...
        if measure_method == "l1_norm":
            distance = -1 * np.mean(
                [abs(q - t) for q, t in zip(query_arr, feat_arr)]
            )  # thêm dấu - để nó đảo chiều cùng mấy độ đo kia
...
def search_obj_count_engine_fast(
    query: str,
    topk: int,
    db: list,
):
    ...
    for num_cls in lst:
        num, cls = num_cls.split("-")
        set_img_of_numobj_cls = db[class_dict[cls.replace("_", " ")]][int(num)]
    ...
    for vid_img in result:
        ...
        if k == topk:
            break
        vid, img = vid_img
        search_result.append({"video_name": vid, "keyframe_id": img, "score": 0})
    return search_result


def query2vec(query, class_names):
    # .e.g: '2-car 4-sink'

    result = np.zeros(len(class_names), dtype="int")

    class2idx = {name: idx for idx, name in enumerate(class_names)}

    for obj_count_pair in query.split():

        count, class_name = obj_count_pair.split("-")

        count = int(count)

        class_idx = class2idx[class_name]

        result[class_idx] += count
...
class ObjectCountSearcher:
    def __init__(self, obj_db: ObjectCountDB):
        ...
        # TODO: reimplement the search_obj_count_engine_slow
        # function with batching
        
    def elastic_search(self, query, topk):
        query_vec = query2vec(query, self.db.class_names)

        return self.db.elastic_search_vec(query_vec, topk)
    ...
    def search_slow(self, query, topk=5, measure_method="l2_norm"):
        return search_obj_count_engine_slow(
            query, self.db.slow_db, topk=topk, measure_method=measure_method
        )
